...
import {GestionPointageAjouterPointageEnginPage} from "../gestion-pointage-ajouter-pointage-engin/gestion-pointage-ajouter-pointage-engin";
...
export class GestionPointageListePointageEnginPage {
  ...
  public nomTableActuelle = "pointagevehicule";


  //la liste des tables suivantes
  public pageDAjout : any = null;
  public pageSuivante : any = GestionPointageAjouterPointageEnginPage;

  public tableauMappingBDD = [
    ["idpointagevehicule","id","number"],
    ["datepointagevehicule","date","date"],
    ["nombreheurepointagevehicule","nombreheure","number"],
    ["refvehiculepointagevehicule","refvehicule","number"],
    ["datedajoutpointagevehicule","datedajout","date"],
    ["refmotifabsencepointagepointagevehicule","refmotifabsencepointage","number"]
  ];
  ...
  refresh(){
    ...
    this.httpClient.get("http://ec2-52-47-166-154.eu-west-3.compute.amazonaws.com:9090/requestAny/" +
      ...
      this.genererLeftJoin(this.nomTableActuelle,"motifabsencepointage") + " " +
      //"left join vehicule where vehicule " +
      ") as tp " +
      "where  tp.idpointage = (select max(P.id) from pointagevehicule as p where p.date = tp.date  " +
      "and p.refvehicule = " + (this.informationsActuelles as any).idvehicule + ")"
    )
      .subscribe(data => {
        ...
        for(let i = 0; i < this.listeObjetActuelle.length ; i++){
          ...
          objetTemp["start"] = this.listeObjetActuelle[i]["datepointagevehicule"];
          if(this.listeObjetActuelle[i]["nombreheurepointagevehicule"] == "0" || this.listeObjetActuelle[i]["nombreheurepointagevehicule"] == "4"){
            objetTemp["title"] = this.listeObjetActuelle[i]["nombreheurepointagevehicule"] + "H";
            objetTemp["color"] = "red";
          ...
          if(this.listeObjetActuelle[i]["nombreheurepointagevehicule"] >= 8){
    ...
    $(this.calendar.nativeElement).fullCalendar({
      ...
      dayClick: function(date, allDay, jsEvent, view) {
        ...
        for(let i = 0; i < _this.listeObjetActuelle.length; i++){
          ...
          if(_this.listeObjetActuelle[i]["datepointagevehicule"] == (date as any)._d.toISOString().substring(0,10)){
            console.log("yes");
        ...
        if((nouvelObjetActuelTemp as any).idpointagevehicule != 'NULL'){
          _this.objetActuel= Object.assign({},nouvelObjetActuelTemp);
        }
        //sinon
        else{

          _this.objetActuel["datepointagevehicule"] = (date as any)._d.toISOString().substring(0,10);
          _this.objetActuel["refvehiculepointagevehicule"] = (_this.informationsActuelles as any).idvehicule;
          _this.objetActuel["nombreheurepointagevehicule"] = 'NULL';
          _this.objetActuel["refmotifabsencepointagepointagevehicule"] = 'NULL';
          _this.objetActuel["idpointagevehicule"] = 'NULL';
          _this.objetActuel["datedajoutpointagevehicule"] = '';
        }
  ...
  updateHeurePointage($event) {
    ...
    for(let i=0 ; i<this.listeObjetActuelle.length;i++){
      if(this.listeObjetActuelle[i]["datedajoutpointagevehicule"] == this.objetActuel["datedajoutpointagevehicule"]){
        if(this.listeObjetActuelle[i]["nombreheurepointagevehicule"] != this.objetActuel["nombreheurepointagevehicule"]){
          (this.objetActuel as any).datedajoutpointagevehicule = (new Date()).toISOString().substring(0,10)+" "+(new Date()).toISOString().substring(11,19);
...
//    _this.oldDayUpdateHeure["day"] = _this.oldDay["day"];
  ...
  updateMotifPointage($event) {
    ...
    for(let i=0 ; i<this.listeObjetActuelle.length;i++){
      if(this.listeObjetActuelle[i]["datedajoutpointagevehicule"] == this.objetActuel["datedajoutpointagevehicule"]){
        if(this.listeObjetActuelle[i]["refmotifabsencepointagepointagevehicule"] != this.objetActuel["refmotifabsencepointagepointagevehicule"]){
          (this.objetActuel as any).datedajoutpointagevehicule = (new Date()).toISOString().substring(0,10)+" "+(new Date()).toISOString().substring(11,19);
          console.log(this.objetActuel);
          this.insertObjet(this.objetActuel,this.nomTableActuelle,this.tableauMappingBDD);
        }
  ...
  marquerAbsent() {
    if(new Date((this.objetActuel as any).datepointagevehicule) <= new Date((new Date()).toISOString().substring(0,10))){
      (this.objetActuel as any).datedajoutpointagevehicule = (new Date()).toISOString().substring(0,10)+" "+(new Date()).toISOString().substring(11,19);
      (this.objetActuel as any).nombreheurepointagevehicule = 0;
      (this.objetActuel as any).refmotifabsencepointagepointagevehicule = 1;
      (this.objetActuel as any).refvehiculepointagevehicule = (this.informationsActuelles as any).idvehicule;
      this.insertObjet(this.objetActuel,this.nomTableActuelle,this.tableauMappingBDD);
    }
    else{
  ...
  marquerPresent() {
    ...
    if(new Date((this.objetActuel as any).datepointagevehicule) <= new Date((new Date()).toISOString().substring(0,10))){
      (this.objetActuel as any).datedajoutpointagevehicule = (new Date()).toISOString().substring(0,10)+" "+(new Date()).toISOString().substring(11,19);
      (this.objetActuel as any).nombreheurepointagevehicule = 8;
      (this.objetActuel as any).refvehiculepointagevehicule = (this.informationsActuelles as any).idvehicule;
      this.insertObjet(this.objetActuel,this.nomTableActuelle,this.tableauMappingBDD);
    }
    else{
