...
export class ListeFournisseurPage {
  ...
  refresh(){
    this.httpClient.get("http://172.20.10.2:9090/requestAny/select " + this.genererListeAttributRequete("fournisseur",this.tableauMappingBDD) + ", string_agg(produitfournisseur.nomproduit,',') as listeproduits from fournisseur LEFT JOIN produitfournisseur ON fournisseur.id = produitfournisseur.reffournisseur group by fournisseur.id order by fournisseur.id desc")
      .subscribe(data => {
