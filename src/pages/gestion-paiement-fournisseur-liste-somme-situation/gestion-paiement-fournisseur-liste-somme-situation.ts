...
import {GestionPaiementFournisseurListePaiementPage} from "../gestion-paiement-fournisseur-liste-paiement/gestion-paiement-fournisseur-liste-paiement";

/**
 * Generated class for the GestionPaiementFournisseurListeSommeSituationPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-gestion-paiement-fournisseur-liste-somme-situation',
  templateUrl: 'gestion-paiement-fournisseur-liste-somme-situation.html',
})
export class GestionPaiementFournisseurListeSommeSituationPage {
  ...
  public pageSuivante : any = GestionPaiementFournisseurListePaiementPage;
  ...
  refresh(){
    ...
    this.httpClient.get("http://ec2-52-47-166-154.eu-west-3.compute.amazonaws.com:9090/requestAny/" +
      "select sd.refchantier, sd.reffournisseur, pf.raisonsocialefournisseur, sum(pf.prixttcproduitfournisseur*article.quantiterecue) as totalttc, sum(pf.prixhtproduitfournisseur*article.quantiterecue) as totalht " +
      "from article left join  " +
        "( select sousdemande.id as idsousdemande ,*  " +
        "from sousdemande  " +
        "left join demande on demande.id = sousdemande.refdemande ) as sd on sd.idsousdemande = article.refsousdemande " +
        "left join ( select produitfournisseur.id as idproduitfournisseur," +
                        "produitfournisseur.prixht * ( 1 + (produitfournisseur.tvaenpourcentage * 0.01)) as prixttcproduitfournisseur," +
                        "produitfournisseur.prixht  as prixhtproduitfournisseur," +
                        "fournisseur.raisonsociale as raisonsocialefournisseur, *  " +
                    "from produitfournisseur  left join fournisseur on fournisseur.id = produitfournisseur.reffournisseur " +
                    "where  " +
                      "not produitfournisseur.prixht is null  " +
                      "and not produitfournisseur.tvaenpourcentage is null ) as pf  " +
        "on pf.idproduitfournisseur=article.refproduitfournisseur " +
      "where  " +
        "not  sd.reffournisseur is null  " +
        "and not article.quantiterecue is null " +
        "and not sd.refchantier is null " +
        "and article.datelivraison between '" + new Date((this.informationsActuelles as any).moisdebut + "-01").toISOString().substring(0,10) + "'" +
      " and '" + new Date((this.informationsActuelles as any).moisfin.substring(0,4) ,
        (this.informationsActuelles as any).moisfin.substring(5,7),
        new Date((this.informationsActuelles as any).moisfin).getDate())
        .toISOString().substring(0,10) + "' "  +
        "and sd.refchantier = 1 group by  sd.reffournisseur, pf.raisonsocialefournisseur,sd.refchantier"
    )
      .subscribe(data => {
