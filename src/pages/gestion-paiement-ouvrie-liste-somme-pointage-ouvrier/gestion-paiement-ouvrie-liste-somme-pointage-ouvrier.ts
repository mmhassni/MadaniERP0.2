...
/**
 * Generated class for the GestionPaiementOuvrieListeSommePointageOuvrierPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-gestion-paiement-ouvrie-liste-somme-pointage-ouvrier',
  templateUrl: 'gestion-paiement-ouvrie-liste-somme-pointage-ouvrier.html',
})
export class GestionPaiementOuvrieListeSommePointageOuvrierPage {
  ...
  refresh(){
    ...
    this.httpClient.get("http://ec2-52-47-166-154.eu-west-3.compute.amazonaws.com:9090/requestAny/" +
      "select tp.salairejournalierouvrier, tp.nomouvrier, tp.refouvrierpointageouvrier, sum(tp.nombreheurepointageouvrier) from (select ouvrier.salairejournalier as salairejournalierouvrier, ouvrier.nom as nomouvrier,  "+this.nomTableActuelle+".id as idpointage, " + this.genererListeAttributRequete(this.nomTableActuelle,this.tableauMappingBDD) + ",* " +
      "from " + this.nomTableActuelle +  "  " +
      this.genererLeftJoin(this.nomTableActuelle,"motifabsencepointage") + " " +
      this.genererLeftJoin(this.nomTableActuelle,"ouvrier")  + ") as tp " +
      "where tp.idpointage = (select max(P.id) from pointageouvrier as p where p.date = tp.date  " +
      "and p.refouvrier = tp.refouvrier" + ")" +
      "and tp.date between '" + new Date((this.informationsActuelles as any).moisdebut + "-01").toISOString().substring(0,10) + "'" +
      ...
      " and tp.refouvrierpointageouvrier in (select refouvrier from chantierouvrierassocie where refchantier = 1) " +
      "group by tp.refouvrierpointageouvrier,tp.nomouvrier,tp.salairejournalierouvrier"
    )
      .subscribe(data => {
