...
/**
 * Generated class for the GestionIntervenantListeIntervenantPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-gestion-intervenant-liste-intervenant',
  templateUrl: 'gestion-intervenant-liste-intervenant.html',
})
export class GestionIntervenantListeIntervenantPage {

  public listeFournisseurs = [];
  public listeFournisseursFiltree = [];

  //non de la table principale de cette page
  public nomTableActuelle = "intervenant";


  //la liste des tables suivantes
  public pageDAjout : any = GestionIntervenantAjouterIntervenantPage;
  public pageSuivante : any = null;

  public tableauMappingBDD = [
    ["idintervenant","id","number"],
    ["raisonsocialeintervenant","raisonsociale","text"],
    ["nomintervenant","nom","text"]
  ];
  ...
  getItems(ev) {
    ...
    if (val && val.trim() != '') {
      this.listeFournisseursFiltree = this.listeFournisseursFiltree.filter((item) => {
        return ( (item.raisonsocialeintervenant + item.nomintervenant).toLowerCase().indexOf(val.toLowerCase()) > -1);
      })
    }
  }
